...
public class Wrapped extends Entity {
    ...
    @Override public boolean isBooster() { return true; }
    ...
    @Override public List<Block> getDestroyables(Board b) {
        ...
        for (int j = column - 1; j <= column + 1; j++) for (int i = row - 1; i <= row + 1; i++) {
            if (i >= 0 && i < Board.getRows() && j >= 0 && j < Board.getColumns() && !b.getBlock(i, j).getEntity()
                    .isVisited()) {
